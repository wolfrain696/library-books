...
export const Library: FC = observer(() => {
    const { favorites, filteredFavorites, sidebar } = FavoritesStore
    const { currentPage, description, category } = DescriptionStore
    ...
    if (description && description.type?.key !== undefined && currentPage) {
        if (
            description.type.key === '/type/work' &&
            BookInfoGuard(description)
        ) {
            elementDescription = (
                <Book
                    favorites={favorites}
            ...
            description.type.key === '/type/author' &&
            AuthorInfoGuard(description)
        ) {
            elementDescription = (
                <Author
                    favorites={favorites}
    ...
    return (
        <div className={S.container}>
            ...
            <main
                onClick={() => FavoritesStore.changeSidebar(false)}
                className={S.body}
            >
                <div
                    onClick={(e) => e.stopPropagation()}
                    className={classNames(S.sidebar, [sidebar && S.active])}
                >
