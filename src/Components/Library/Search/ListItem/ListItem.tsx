...
export const ListItem: FC<ListProps> = ({
    ...
    let favoriteStatus =
        favorites.filter((el) => el.page.key === item.key).length === 1
    ...
    return (
        <li
            className={page === item ? S.card + ' ' + S.active : S.card}
            onClick={() => changePage(item, url)}
        >
            <div className={S.avatarBox}>
                <img
                    className={S.avatar}
                    src={`http://covers.openlibrary.org/${
                        img ? 'b' : 'a'
                    }/olid/${img ? img : authorPhoto}-M.jpg`}
                    alt="book card"
                />
            ...
            <div className={S.textBox}>
                <div className={S.mainText}>{title ? title : name}</div>
                <div className={S.secondaryTextBox}>
                    <div className={S.secondaryText}>{title}</div>
                    {favoriteStatus && category !== 'favorites' && (
