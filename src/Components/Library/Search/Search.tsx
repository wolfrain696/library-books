...
export const Search: FC<searchProps> = observer(
    ({
        ...
        useEffect(() => {
            if (fetching) DescriptionStore.lazyData()
        }, [fetching])
        ...
        const scrollHandler = (e: React.BaseSyntheticEvent) => {
            ...
            if (
                scrollHeight - (scrollTop + 500) < 100 &&
                (data.length < totalCount ||
                    (category === 'default' && data.length < 100))
            )
        ...
        return (
            <div className={S.content}>
                {category === 'default' ? (
                    <div className={S.default_title}>
                        Топ 100 классических книг
                    </div>
                ) : (
