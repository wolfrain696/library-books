...
export const Book: FC<BookProps> = ({ page, info, favorites }) => {
    let favoriteStatus: boolean =
        favorites.filter((el) => el.page.title === page.title).length === 1
        ...
        <div className={S.description}>
            ...
            <div className={S.top}>
                ...
                <div className={S.content}>
                    <h1 className={S.h1}>{info.title}</h1>
                    <p
                        className={S.link}
                        onClick={() => selectAuthor(info.authors)}
                    >
                        Автор:{' '}
                        {page.authors ? page.authors[0].name : page.author_name}
                    </p>
                    {page.publish_date && (
