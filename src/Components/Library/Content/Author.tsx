...
export const Author: FC<AuthorProps> = ({ page, info, favorites }) => {
    let favoriteStatus: boolean =
        favorites.filter((el) => el.page.name === page?.name).length === 1

    const listOfWorks = (val: string) => {
        DescriptionStore.addAuthorBooks(val)
        window.innerWidth < 800 && DescriptionStore.removeDescription()
        ...
        <div className={S.description}>
            ...
            <div className={S.top}>
                ...
                <div className={S.content}>
                    <h1 className={S.h1}>{info?.name}</h1>
                    <p>Дата рождения: {info?.birth_date}</p>
                    <p
                        className={S.link}
                        onClick={() => listOfWorks(info?.name)}
                    >
                        Список работ
                    </p>
                    {favoriteStatus ? (
                        <button
                            onClick={() =>
                                FavoritesStore.removeFavorites(page?.key)
                            }
                        >
                            <img
                                src={favoriteActive}
                                alt="like"
                                className={S.like_book}
                            />
                        </button>
                    ) : (
                        <button
